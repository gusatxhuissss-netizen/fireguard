...
export function RiskMap({ incidents, reports, visibleRisks }: { incidents: Incident[]; reports: Report[]; visibleRisks: RiskLevel[] }) {
  ...
  const minLat = Math.min(...latitudes, -15.88) - .01;
  const maxLat = Math.max(...latitudes, -15.72) + .01;
  const minLng = Math.min(...longitudes, -47.97) - .01;
  const maxLng = Math.max(...longitudes, -47.82) + .01;
  ...
  return <div className="relative isolate h-[430px] overflow-hidden rounded-[1.25rem] border border-[#b9cdb9] bg-[#d6e3cb]">
    ...
    <div className="absolute left-5 top-5 flex items-center gap-2 rounded-lg border border-white/75 bg-[#f7f9f1]/85 px-3 py-2 text-[#31594b] shadow-sm"><ScanSearch className="h-3.5 w-3.5" /><span className="font-mono text-[9px] font-bold uppercase tracking-[.14em]">Camada territorial · demo</span></div>
